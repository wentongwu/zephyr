...
or:
    ...
    cmake -DBOARD=sam_e70_xplained -DCONF_FILE=prj_uart_data.conf ..
...
and is used as the tracing backend.
...
or:
    ...
    cmake -DBOARD=sam_e70_xplained -DCONF_FILE=prj_usb_data.conf ..
...
Build a POSIX-tracing image with:
    ...
    cmake -DBOARD=native_posix -DCONF_FILE=prj_posix.conf ..

or:

    cmake -DBOARD=native_posix -DCONF_FILE=prj_posix_data.conf ..

After the application has run for a while, check the trace file named log_posix.
